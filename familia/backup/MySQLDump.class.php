...
class MySQLDump
{
   ...
   * @access public
   */
  public function __construct($host, $dbUser, $dbPass, $dbName, $dest,
      $zip = 'gz')
  {
    $zip_util = array('gz'=>'gzip','bz2'=>'bzip2');
    if (array_key_exists($zip, $zip_util))
    {
      $fname = $dbName . '_' . date("md") . '.sql.' . $zip; 
      $this->cmd = 'mysqldump -u' . $dbUser . ' -h '.$host .' -p' . $dbPass .
          ' ' . $dbName . '| ' . $zip_util[$zip] . ' >' .
          $dest . '/' . $fname;
    }
    else
    {
      $fname = $dbName . '_' . date("md") . '.sql'; 
      $this->cmd = 'mysqldump -u' . $dbUser . ' -h '.$host .' -p' . $dbPass .
          ' ' . $dbName . ' >' . $dest . '/' . $fname;
    }
  }
